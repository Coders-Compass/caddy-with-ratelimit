...
- **SPDX SBOM** generated by BuildKit and attached as an OCI artifact on the manifest. Inspect with:
  ```bash
  docker buildx imagetools inspect ghcr.io/coders-compass/caddy-with-ratelimit:latest --format '{{ json .SBOM }}'
  ```
- **SLSA build provenance** (`mode=max`) attached via BuildKit plus a signed Sigstore attestation via `actions/attest-build-provenance`. Verify with `gh attestation verify` or `cosign verify-attestation`.
- **Trivy vulnerability scan** runs on every build before push. Builds fail on CRITICAL findings; HIGH/MEDIUM/LOW are reported. Results are uploaded to the repository's [Code Scanning](https://github.com/Coders-Compass/caddy-with-ratelimit/security/code-scanning) tab and attached to each workflow run as a `trivy-results-*` artifact (SARIF + table, 90-day retention). Suppressions live in [`.trivyignore`](.trivyignore).
